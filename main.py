...
class DocumentApp:
    ...
    def create_widgets(self):
        ...
        # Подзаголовок с поддерживаемыми форматами
        formats_label = ttk.Label(
            main_frame,
            text="Поддерживаемые форматы: .txt, .pdf, .doc, .docx",
            font=("Arial", 10),
            foreground="blue"
        )
        formats_label.grid(row=1, column=0, columnspan=3, pady=(0, 10))
        ...
        # Панель выбора файла
        file_panel = ttk.LabelFrame(main_frame, text="Выбор документа", padding="10")
        file_panel.grid(row=3, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 20))
        ...
        self.file_label.grid(row=0, column=0, columnspan=2, sticky=(tk.W, tk.E), padx=(0, 10))
        ...
        self.file_info_frame.grid(row=2, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(5, 0))
        ...
        self.filename_label.grid(row=0, column=0, columnspan=4, sticky=tk.W, pady=(0, 5))
        ...
        for i, (label, var) in enumerate([
            ...
            self.file_info_labels[var].grid(row=1, column=i*2+1, sticky=tk.W, padx=(0, 20))
            setattr(self, var, var_obj)
        
        # Панель действий
        action_panel = ttk.LabelFrame(main_frame, text="Действия с документом", padding="15")
        action_panel.grid(row=4, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 20))
        
        buttons_frame = ttk.Frame(action_panel)
        buttons_frame.grid(row=0, column=0, pady=(0, 10))
        ...
        self.add_button.pack(side=tk.LEFT, padx=(0, 15))
        ...
        stats_panel = ttk.LabelFrame(main_frame, text="📊 Статистика базы данных", padding="10")
        stats_panel.grid(row=5, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 20))
        
        self.stats_label = ttk.Label(
            stats_panel,
            text="Документов в базе: 0",
            font=("Arial", 10)
        )
        ...
        # Список документов
        docs_panel = ttk.LabelFrame(main_frame, text="📄 Документы в базе данных", padding="10")
        ...
        self.doc_tree = ttk.Treeview(docs_panel, columns=columns, 
                                    show='headings', height=8)
        
        # Определение заголовков
        self.doc_tree.heading('ID', text='ID')
        self.doc_tree.heading('Название документа', text='Название документа (имя файла)')
        ...
        # Полоса прокрутки
        scrollbar = ttk.Scrollbar(docs_panel, orient=tk.VERTICAL, 
        ...
        docs_buttons_frame.grid(row=1, column=0, columnspan=2, pady=(10, 0), sticky=tk.E)
    ...
    def update_file_info(self, file_path):
        ...
        try:
            ...
            filename = file_info.get('filename_without_ext', os.path.splitext(os.path.basename(file_path))[0])
            ...
            # Если это не TXT файл, показываем иконку конвертации
            ext = file_info['extension']
            if ext != '.txt':
                self.update_status(f"📄 Файл {ext.upper()} будет автоматически конвертирован")
    ...
    def show_file_error(self, error_message: str):
        ...
        self.update_status(f"⚠️ Ошибка чтения файла: {error_message[:50]}...")
    ...
    def add_to_db(self):
        ...
        if not messagebox.askyesno("Подтверждение", 
                                  f"Добавить документ '{filename}' в базу данных?"):
            return
    ...
    def _add_to_db_thread(self, file_path):
        ...
        try:
            self.root.after(0, lambda: self.show_loading(True, "Обработка документа..."))
            ...
            if result['success']:
                ...
                self.root.after(0, lambda: self.update_status(
                    f"✅ Документ '{result['label']}' успешно добавлен (ID: {result['doc_id']})"
                ))
                ...
                self.root.after(0, lambda: self.update_status(f"❌ Ошибка: {result['message']}"))
    ...
    def search_relevant(self):
        ...
        # Запуск в отдельном потоке
        thread = threading.Thread(target=self._search_relevant_thread, args=(file_path,))
        thread.daemon = True
        thread.start()
    ...
    def _search_relevant_thread(self, file_path):
        ...
        try:
            self.root.after(0, lambda: self.show_loading(True, "Поиск релевантных документов..."))
            ...
            if relevant_papers:
                ...
                for label, similarity in relevant_papers:
                    ...
                    if similarity_percent >= 60:
                        formatted_results.append(f"{label} (сходство: {similarity_percent:.1f}%)")
                
                # Открытие окна с результатами
                self.root.after(0, self.show_results_window, formatted_results)
                self.root.after(0, lambda: self.update_status(
                    f"🔍 Найдено документов: {len(relevant_papers)}"
                ))
            else:
                self.root.after(0, lambda: self.update_status("Релевантные документы не найдены"))
                self.root.after(0, lambda: messagebox.showinfo(
                    "🔍 Результаты поиска",
    ...
    def copy_to_clipboard(self, results: List[str], window: tk.Toplevel):
        ...
        if results:
            ...
            messagebox.showinfo("✅ Успешно", "Список скопирован в буфер обмена", parent=window)
        else:
            messagebox.showwarning("⚠️ Внимание", "Нет результатов для копирования", parent=window)
    
    def save_results_to_file(self, results: List[str], window: tk.Toplevel):
        """Сохранение результатов в файл"""
        if not results:
            messagebox.showwarning("⚠️ Внимание", "Нет результатов для сохранения", parent=window)
            return
        ...
        if file_path:
            try:
                ...
                messagebox.showinfo("✅ Успешно", f"Результаты сохранены в файл:\n{file_path}", parent=window)
            except Exception as e:
    ...
    def refresh_document_list(self):
        ...
        try:
            ...
            for doc in all_docs[-15:]:
                keywords_str = ", ".join(doc['keywords'][:3])  # Показываем первые 3 ключевых слова
    ...
    def delete_selected_document(self):
        ...
        if messagebox.askyesno("⚠️ Подтверждение удаления", 
            ...
            try:
                ...
                if self.db.delete_document(doc_id):
                    self.update_status(f"✅ Документ '{doc_name}' удален")
                    self.refresh_document_list()
                    self.update_stats()
                    messagebox.showinfo("✅ Успешно", f"Документ '{doc_name}' удален из базы данных")
                else:
